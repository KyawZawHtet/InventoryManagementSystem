...
public class LotRepository {
	...
	public int insertLot(LotDto dto) {
        ...
        try {
        	...
        	PreparedStatement checkPs = con.prepareStatement(
                    "SELECT id, quantity FROM lot WHERE expired_date_date = ? AND price = ?"
                );
                ...
                PreparedStatement insertPs = con.prepareStatement(
                    "INSERT INTO lot (product_id, lot_no, expired_date_date, price, date, uom, quantity)" +
                    "VALUES (?, ?, ?, ?, ?, ? ,?)"
                );
    ...
    public int updateLot(LotDto dto) {
        ...
        try {
            PreparedStatement ps = con.prepareStatement(
                "UPDATE lot SET product_id = ?, expired_date = ?,price = ?,date = ?,quantity = ?" +
                "WHERE lot_no = ?"
            );
            ...
            ps.setDouble(5, dto.getQuantity());
	...
	public void updateQty(String key, int decreaseQty) {
		...
		try {
			...
			System.out.println("Decreate Quantity : "+ e.getMessage());
		}
	}
